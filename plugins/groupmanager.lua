...
local function modrem(msg)
  ...
  if not data[tostring(msg.to.id)] then
    return '‏*>* _المجموعة 🚻 :-_* ['..msg.to.title..']*_ تم تعطيلها مسبقاً 📴  !_'
  end
...
.."\n*>* تشغيل التحذير ⛔️:- "..settings.lock_woring
 ...
 .."\n*قناتنا 📢 :* @Star_Wars\n"
.."\n*تواصل معنا 📨 :* @SuperWars_Bot\n"

 list_settings = "* اعدادات المجموعه ⚙️ "
...
if matches[2] == "الكل" then
 ...
 local text =  '*>* _تم قفل 🔐 جميع الوسائط 🔕 في المجموعة ✅'
...
if matches[2] == "الكل" then
 ...
 unmute_inline(msg ,data, target)
    ...
    local text =  '*>* _تم فتح 🔓 جميع الوسائط 🔔 في المجموعة ✅' 
tdcli.sendMessage(msg.to.id, msg.id, 1, text, 0, "md")    
return open_all
end
if matches[2] == "المتحركه" then
...
return "*>* _عزيزي 😁 تم ارسال الرابط في الخاص 📥 بنجاح ✅ !_"
...
if matches[1] == "ايقاف" and is_mod(msg) then
    ...
    welcome = data[tostring(chat)]['settings']['welcome']
	if welcome == "❌" then
	return "*>* تم ايقاف الترحيب 💌 مسبقاً ❌_"
			else
		data[tostring(chat)]['settings']['welcome'] = "❌"
	    save_data(_config.moderation.data, data)
return "*>* تم ايقاف الترحيب 💌 بنجاح ❌_"
			end
end
    ...
    lock_woring = data[tostring(chat)]['settings']['lock_woring']
	if lock_woring == "❌" then
	return "*>* تم ايقاف التحذير 📵 مسبقاً ❌_"
			else
		data[tostring(chat)]['settings']['lock_woring'] = "❌"
	    save_data(_config.moderation.data, data)
return "*>* تم ايقاف التحذير 📵 بنجاح ❌_"
			end
	end
	end
...
local data = load_data(_config.moderation.data)
 if checkmod and msg.text and msg.to.type == 'channel' then
	...
	local secchk = true
		...
		if secchk then
			checkmod = false
return tdcli.sendMessage(msg.to.id, 0, 1, '*>* _البوت ليس ادمن في المجموعة 🚻 من فضلك 😄 يرجى ترقيتة ادمن لكي يقوم بلعمل ⚡️ !', 1, "md")
		end
	end, nil)
 end
